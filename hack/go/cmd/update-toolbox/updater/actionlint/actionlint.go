...
func verify(ctx context.Context, clients *updater.Clients, release *github.Release) (toolbox.Downloads, error) {
	...
	assets := updater.AssetsToDownload{
		platform.DarwinARM64: {
			Name:    fmt.Sprintf("actionlint_%s_darwin_arm64.tar.gz", version),
			Extract: "actionlint",
		},
		platform.LinuxARM64: {
			Name:    fmt.Sprintf("actionlint_%s_linux_arm64.tar.gz", version),
			Extract: "actionlint",
		},
		platform.LinuxX64: {
			Name:    fmt.Sprintf("actionlint_%s_linux_amd64.tar.gz", version),
			Extract: "actionlint",
		},
	}

	digestsAsset := fmt.Sprintf("actionlint_%s_checksums.txt", version)
