...
func verify(ctx context.Context, clients *updater.Clients, release *github.Release) (toolbox.Downloads, error) {
	...
	assets := updater.AssetsToDownload{
		platform.DarwinARM64: {
			Name:    fmt.Sprintf("helmfile_%s_darwin_arm64.tar.gz", version),
			Extract: "helmfile",
		},
		platform.LinuxARM64: {
			Name:    fmt.Sprintf("helmfile_%s_linux_arm64.tar.gz", version),
			Extract: "helmfile",
		},
		platform.LinuxX64: {
			Name:    fmt.Sprintf("helmfile_%s_linux_amd64.tar.gz", version),
			Extract: "helmfile",
		},
	}
